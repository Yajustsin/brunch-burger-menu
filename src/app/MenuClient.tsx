...
import type { Category, MenuItem, RestaurantInfo, MenuData } from "@/lib/db";
...
/* ── Menu Item Card ── */
function MenuItemCard({ item, delay = 0 }: { item: MenuItem; delay?: number }) {
    ...
    fetch(BASE + "/api/menu")
      ...
      .then((data) => {
        if (data) {
          ...
          if (data.items) setItems(data.items as MenuItem[]);
        }
      })
      .catch(() => {});
  }, []);
      ...
      <header className="sticky top-0 z-40 paper-header">
        <div className="pt-5 pb-3 px-5">
          <div className="text-center mb-4">
            ...
            <h1 className="text-xs font-bold text-ink-600 mt-2 tracking-wide">
              منوی آنلاین برانچ برگر
            </h1>
            {restaurant && (
              <p className="ltr text-ink-500 text-[11px] mt-1.5 tracking-wider">
        ...
        <div className="flex flex-col gap-2.5 mt-4" key={activeCategory}>
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item, idx) => (
              <MenuItemCard key={item.id} item={item} delay={Math.min(idx * 60, 360)} />
            ))
          )}
        </div>
      </main>
